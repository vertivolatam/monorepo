...
| [`2026-06-21-dashboard-fleet-cockpit`](./changes/2026-06-21-dashboard-fleet-cockpit/) | proposal · design · tasks | Proposed — pendiente aprobación. Cablear `vertivo_dashboard` (Jaspr + D3) al backend Serverpod como cockpit de flota: dep `vertivo_client`, capa de repositories, 6 pages → endpoints reales (`greenhouse.listByUser`/`getReadings`, `alert.*`, `anomaly.*`, `cropCatalog.*`), multi-tenant por sesión auth, InstrumentCard para rangos. Materializa CLAUDE.md regla #2 "Connect before create". Issue: VRTV-99. |
